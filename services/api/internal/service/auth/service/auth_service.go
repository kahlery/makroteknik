...
func (a *AuthService) Login(c *fiber.Ctx) error {
	...
	if err := c.BodyParser(&req); err != nil {
		pkgLog.LogError("Error in parsing request body: %v"+err.Error(), "AuthService.Login")
		return c.Status(fiber.StatusBadRequest).SendString("Invalid input")
	}

	user, err := a.userRepo.FindByUserName(c.Context(), req.Username)
	if err != nil {
		pkgLog.LogError("Error in parsing request body: %v"+req.Username+req.Password+err.Error(), "AuthService.Login()")
