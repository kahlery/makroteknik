...
const ListingGrid = ({ productsList, featured }) => {
    return (
        <div className="grid lg:grid-cols-2 xl:grid-cols-3 sm:gap-4 border-x">
            ...
            {productsList.map((product) => (
                <div key={product.code} className="flex flex-col bg-white bg-opacity-80 p-4 text-sm hover:scale-105 hover:-translate-y-5 duration-500 h-90 overflow-hidden shadow">
                    ...
                    <div className="flex justify-between sm:mt-auto items-center align-bottom gap-4">
                        ...
                        <button className="shadow border text-black rounded-md p-2 mt-4 w-full text-xs">View details</button>
                    </div>
                </div>
            ))}
        </div>
